...
FIL = os.path.join(HERE, "data", "inventering.json")
LÅS = threading.RLock()
...
def _stada_falt(kropp, ny=False):
    ...
    for nyckel, typ in FALT.items():
        ...
        v = kropp[nyckel]
        if typ is float:
            # "18 500", "1 250,50" och "1,250.50" ska alla bli tal.
            # Vanligt mellanslag och hårt mellanslag är tusentalsavgränsare.
            text = (str(v).replace(" ", "").replace(" ", "")
                    .replace(" ", "").replace("kr", "").replace(":-", "").strip())
            # Både "1 250,50" och "1.234,50" förekommer. Den avgränsare som
            # står sist är decimaltecknet — de andra är tusental.
...
def rakna(pid, delta):
    ...
    for post in data["poster"]:
        if post["id"] == pid:
            ...
            efter = max(0.0, före + float(delta))
            if efter == int(efter):
                efter = int(efter)
            post["antal"] = efter
            post["andrad"] = time.strftime("%Y-%m-%d %H:%M")
